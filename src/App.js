...
function FileDrop({ onUpload, label }) {
  ...
  return (
    <div
      style={{
        border: `2px dashed ${hover ? C.blue : C.border}`,
        borderRadius: 8,
        padding: "1rem",
        textAlign: "center",
        cursor: "pointer",
        background: hover ? C.blueBg : C.bg,
        marginBottom: 8,
    ...
    >
      <input ref={inpRef} type="file" accept=".csv" style={{ display: "none" }} onChange={(e) => handleFile(e.target.files[0])} />
      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.65rem", color: C.sub }}>{hover ? "Drop CSV here" : label}</span>
    </div>
  );
}
...
function FloorMap({log,dept,staffDate,shiftType, laborShareEnabled, laborShareCount, crossDeptUsage, incrementCrossDept, getCrossDeptUsed}) {
  ...
  const delPath = (pid) => {
    if (!window.window.window.window.confirm("Delete path and all its lines/stations?")) return;
    if (preStaffMode && preStaffShift) {
      const cfg = getShiftConfig(preStaffShift);
  ...
  const delLine = (lid) => {
    if (!window.window.window.window.confirm("Delete line and all its stations?")) return;
    if (preStaffMode && preStaffShift) {
      const cfg = getShiftConfig(preStaffShift);
  ...
  function doAssign(override=false){
    ...
    // Update shiftAssignments so that recall/switchHalf reflects this manual assignment
    const sKey = shiftKey(dept, dateStr, shiftType);
    if (!shiftAssignments[sKey]) shiftAssignments[sKey] = {};
    if (!shiftAssignments[sKey][assoc.badge]) {
      shiftAssignments[sKey][assoc.badge] = { date: dateStr, half1: null, half2: null, dept, shiftType };
    }
    const hd = {
      path: manModal.pathName,
      station: { id: manModal.sid, name: manModal.stName, line_name: lineObj?.name || '' },
      score: 0,
      lc: 0,
      roleType: "MANUAL",
      rotationHours: 10,
    ...
    shiftAssignments[sKey][assoc.badge][showingHalf] = hd;

    // Also update weekHistory for rotation tracking
    if (!weekHistory[assoc.badge]) weekHistory[assoc.badge] = [];
    weekHistory[assoc.badge].push({
      date: dateStr,
      pathName: manModal.pathName,
      roleType: "MANUAL",
      half: showingHalf,
      dept: dept,
      shiftType: shiftType
    ...
    return(
      <div key={st.id}
        ...
        <div style={{display:"flex",gap:3,marginTop:4}}>
          ...
          <button onClick={e=>{e.stopPropagation();if(window.window.window.window.confirm("Delete station?"))delSt(st.id);}} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"0.5rem",color:C.red,padding:"1px 3px"}}>✕</button>
        </div>
      </div>
    );
  };
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem",flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,background:localLaborShareEnabled?C.purpleBg:C.bg,border:`1.5px solid ${localLaborShareEnabled?C.purpleBorder:C.border}`,borderRadius:8,padding:"3px 8px"}}>
            <button onClick={()=>{ const newVal = !localLaborShareEnabled; setLocalLaborShareEnabled(newVal); }} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"0.58rem",fontWeight:600,color:localLaborShareEnabled?C.purple:C.muted,letterSpacing:"0.06em"}}>
...
function AssociatesFull({dept}) {
  ...
  const deleteAssociate = async (badge, name) => {
    if (!window.window.confirm(`Are you sure you want to delete ${name} (${badge})? This cannot be undone.`)) return;
...
function PerformanceTab() {
  ...
  const fetchRates = async () => {
    const res = await api("/performance/latest");
    if (res) setRates(res);
  };

  useEffect(() => {
    fetchRates();
    const t = setInterval(fetchRates, 5000);
    return () => clearInterval(t);
  }, []);

  const uploadRates = async (data) => {
    const res = await api("/performance/upload", { method: "POST", body: JSON.stringify({ csv: data }) });
    if (res && !res.error) { setMsg("Rates uploaded"); fetchRates(); } else setMsg("Error uploading rates");
  };
  const uploadTargets = async (data) => {
    const res = await api("/performance/targets", { method: "POST", body: JSON.stringify({ csv: data }) });
    if (res && !res.error) { setMsg("Targets uploaded"); fetchRates(); } else setMsg("Error uploading targets");
  };

  const lines = [...new Set(rates.map(r => r.line_name).filter(Boolean))].sort();

  return (
    <div style={{ padding: "1.5rem", height: "100%", overflowY: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.58rem", color: C.muted, letterSpacing: "0.25em" }}>REAL‑TIME PERFORMANCE</div>
        <Btn size="sm" onClick={() => setShowUpload(!showUpload)}>{showUpload ? "Hide CSV Upload" : "Upload CSV"}</Btn>
      </div>
      {msg && <div style={{ background: C.greenBg, border: `1px solid ${C.greenBorder}`, padding: "0.5rem", marginBottom: 8, fontFamily: "'DM Mono',monospace", fontSize: "0.65rem", borderRadius: 6 }}>{msg}</div>}
      {showUpload && (
        <div style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "1rem", marginBottom: 12 }}>
          <FileDrop onUpload={uploadRates} label="Drop Rates CSV (Login, Rate, Station)" />
          <FileDrop onUpload={uploadTargets} label="Drop Targets CSV (Station, TargetRate)" />
        </div>
      )}
      {lines.map(line => {
        const lineRates = rates.filter(r => r.line_name === line);
        return (
          <div key={line} style={{ marginBottom: "1.5rem" }}>
            <div style={{ fontFamily: "'Bebas Neue',Impact,sans-serif", fontSize: "1.2rem", color: C.text, marginBottom: 8 }}>{line}</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8 }}>
              {lineRates.map(r => {
                const pct = r.target_rate > 0 ? (r.rate / r.target_rate) * 100 : 100;
                const color = pct >= 100 ? C.green : pct >= 80 ? C.amber : C.red;
                return (
                  <div key={r.station_id} style={{ background: C.surface, border: `1.5px solid ${color}`, borderRadius: 10, padding: "0.6rem", cursor: "default" }}>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.7rem", fontWeight: 700, color: C.text, marginBottom: 4 }}>{r.login}</div>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.55rem", color: C.muted }}>{r.station_name}</div>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.7rem", fontWeight: 600, color }}>{r.rate} / {r.target_rate} ({pct.toFixed(0)}%)</div>
                  </div>
                );
              })}
            </div>
      ...
      {rates.length === 0 && <div style={{ textAlign: "center", padding: "2rem", fontFamily: "'DM Mono',monospace", fontSize: "0.7rem", color: C.faint }}>No performance data. Upload rates & targets.</div>}
    </div>
  );
}
...
function AlertsTab() {
  ...
  const categories = ["ALL", "Small", "Medium", "Large", "C-Returns"];
  ...
  useEffect(() => { fetchAlerts(); const t = setInterval(fetchAlerts, 10000); return () => clearInterval(t); }, [filter]);

  const uploadCSV = async (data) => {
    const res = await api("/alerts/upload", { method: "POST", body: JSON.stringify({ csv: data }) });
    if (res && !res.error) { setUploadMsg(`Inserted ${res.inserted} alerts`); fetchAlerts(); } else setUploadMsg("Error uploading");
  };

  const acknowledge = async (id) => {
    await api(`/alerts/${id}`, { method: "PATCH", body: JSON.stringify({ acknowledged: true }) });
    fetchAlerts();
  };
  const resolveAlert = async (id) => {
    await api(`/alerts/${id}`, { method: "PATCH", body: JSON.stringify({ resolved: true }) });
    fetchAlerts();
  };
  ...
  const saveConfig = async () => {
    if (!newCategory.trim() || isNaN(parseFloat(newThreshold))) return setConfigMsg("Invalid input");
    await api("/alerts/config", { method: "POST", body: JSON.stringify({ category: newCategory.trim(), threshold_percent: parseFloat(newThreshold) }) });
    setNewCategory(""); setNewThreshold("10"); setConfigMsg("Saved"); fetchConfig();
  };
    ...
    <div style={{ padding: "1.5rem", height: "100%", overflowY: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <Btn size="sm" onClick={() => setShowConfig(!showConfig)}>⚙ Config</Btn>
          <Btn size="sm" onClick={() => setShowUpload(!showUpload)}>Upload CSV</Btn>
        </div>
      </div>

      {showConfig && (
        <div style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "1rem", marginBottom: 12 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Alert Thresholds</div>
          {config.map(c => (
            <div key={c.category} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span>{c.category}</span>
              <span>{c.threshold_percent}% below plan</span>
            </div>
          ))}
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            <input value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder="Category" style={{ width: 120, padding: "0.3rem", border: `1px solid ${C.border}`, borderRadius: 4 }} />
            <input value={newThreshold} onChange={e => setNewThreshold(e.target.value)} placeholder="Threshold %" style={{ width: 80, padding: "0.3rem", border: `1px solid ${C.border}`, borderRadius: 4 }} />
            <Btn size="sm" variant="primary" onClick={saveConfig}>Set</Btn>
          </div>
          {configMsg && <div style={{ marginTop: 4, color: C.green, fontSize: "0.7rem" }}>{configMsg}</div>}
        </div>
      )}

      {showUpload && (
        <div style={{ marginBottom: 12 }}>
          <FileDrop onUpload={uploadCSV} label="Drop Alert CSV (Category,Line,Station,PlanRate,ActualRate)" />
          {uploadMsg && <div style={{ color: C.green, fontSize: "0.7rem" }}>{uploadMsg}</div>}
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {alerts.map(alert => (
          <div key={alert.id} style={{
            background: alert.acknowledged ? C.bg : (alert.deviation >= 20 ? C.redBg : C.amberBg),
            border: `1px solid ${alert.acknowledged ? C.border : alert.deviation >= 20 ? C.redBorder : C.amberBorder}`,
            borderRadius: 10,
            padding: "0.6rem 0.9rem",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 6 }}>
              <div>
                <Bdg color={alert.category === "C-Returns" ? C.purple : C.blue} style={{ marginRight: 6 }}>{alert.category}</Bdg>
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.68rem", color: C.text, marginRight: 6 }}>{alert.station_name}</span>
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.55rem", color: C.muted }}>Plan: {alert.plan_rate} | Actual: {alert.actual_rate}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.65rem", color: C.red, fontWeight: 700 }}>{alert.deviation.toFixed(1)}% off</span>
                {!alert.acknowledged && <Btn size="sm" onClick={() => acknowledge(alert.id)}>Ack</Btn>}
                {!alert.resolved && <Btn size="sm" variant="danger" onClick={() => resolveAlert(alert.id)}>Resolve</Btn>}
                {alert.resolved && <Bdg color={C.green} bg={C.greenBg} border={C.greenBorder}>Resolved</Bdg>}
              </div>
            </div>
          </div>
        ))}
        {alerts.length === 0 && <div style={{ textAlign: "center", padding: "2rem", color: C.faint }}>No alerts</div>}
      </div>
    </div>
  );
}
...
function LeaderboardTab() {
  ...
  const fetchLeaderboard = async () => {
    ...
    if (res) setLeaderboard(res);
  };

  useEffect(() => { fetchLeaderboard(); const t = setInterval(fetchLeaderboard, 30000); return () => clearInterval(t); }, []);

  const uploadQuality = async (data) => {
    const res = await api("/quality/upload", { method: "POST", body: JSON.stringify({ csv: data }) });
    if (res && !res.error) { setMsg("Scores uploaded"); fetchLeaderboard(); } else setMsg("Error");
  };

  return (
    <div style={{ padding: "1.5rem", height: "100%", overflowY: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <div>HIGH‑SIDE LEADERBOARD</div>
        <Btn size="sm" onClick={() => setShowUpload(!showUpload)}>Upload Scores</Btn>
      </div>
      {showUpload && (
        <div style={{ marginBottom: 12 }}>
          <FileDrop onUpload={uploadQuality} label="Drop Quality CSV (Badge, Score)" />
          {msg && <div style={{ color: C.green, fontSize: "0.7rem" }}>{msg}</div>}
        </div>
      )}
      <div style={{ fontSize: "0.7rem", color: C.muted, marginBottom: 10 }}>Top 10 – most high‑side hours this month (LC ≥ 2, tenure ≥ 30d, quality ≥ 80)</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {leaderboard.map((a, idx) => (
          <div key={a.badge} style={{
            display: "flex", alignItems: "center", gap: 12,
            background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "0.7rem 1rem"
          }}>
            <div style={{ fontFamily: "'Bebas Neue',Impact,sans-serif", fontSize: "1.5rem", color: C.amber, width: 30 }}>#{idx + 1}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.75rem", fontWeight: 700 }}>{a.name}</div>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.55rem", color: C.muted }}>{a.login}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.8rem", color: C.blue, fontWeight: 700 }}>{a.total_hours.toFixed(1)}h</div>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "0.55rem", color: C.muted }}>LC {a.lc_level} · Q {a.quality_score}</div>
            </div>
          </div>
        ))}
        {leaderboard.length === 0 && <div style={{ textAlign: "center", padding: "2rem", color: C.faint }}>No eligible associates this month</div>}
      </div>
    </div>
  );
}
...
function Settings({onAdminChange, apiUrl, userRole}) {
  ...
  async function removeAdmin(id) {
    if (!isAdmin) return;
    if (!window.window.window.window.confirm("Remove this admin profile?")) return;
...
export default function App(){
  ...
  async function resetShift(){
    await api("/reset-shift",{method:"POST"});
    const dateStr=staffDate instanceof Date?staffDate.toISOString().split("T")[0]:staffDate;
    const ctx=`${dateStr}|${shiftType}|${dept}`;
    clearContextAssignments(ctx);
    // Clear shiftAssignments for the current dept/date/shift so scans after reset start fresh
    delete shiftAssignments[shiftKey(dept, dateStr, shiftType)];
    resetShiftConfig(shiftType);
    setLog([]);
    setResetModal(false);
  }
  ...
  const TABS = [
    ...
    {id:"map",label:"FLOOR MAP",icon:"🗺"},
    {id:"perms",label:"PERMISSIONS",icon:"🔐"},
    {id:"assocs",label:"ASSOCIATES",icon:"👥"},
