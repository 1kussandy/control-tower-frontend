...
//   };
//   const delPath = (pid) => {
//     if (!window.window.confirm("Delete path and all its lines/stations?")) return;
//     if (preStaffMode && preStaffShift) {
//       const cfg = getShiftConfig(preStaffShift);
...
//   };
//   const delLine = (lid) => {
//     if (!window.window.confirm("Delete line and all its stations?")) return;
//     if (preStaffMode && preStaffShift) {
//       const cfg = getShiftConfig(preStaffShift);
...
//           <button onClick={e=>{e.stopPropagation();if(window.window.confirm("Delete station?"))delSt(st.id);}} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"0.5rem",color:C.red,padding:"1px 3px"}}>✕</button>
//         </div>
//       </div>
//     );
//   };
...
//     if(!window.window.confirm("Remove this admin profile?"))return;
